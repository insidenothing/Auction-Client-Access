...
<fieldset>
	<legend><?=$user[name]?>'s Auction Desktop</legend>
		<li><a href="dailyInvoices.php?uid=<?=$_GET[uid]?>">Daily Invoice Lists</a> <small>[added 4/16/2009]</small></li>

		<? if ($user[attorneys_id] == "1"){?>
			<li><a href="csvtesting.php?uid=<?=$_GET[uid]?>">Test Exported CSV against Harvey West Database</a></li>
			<li><a href="upload-burson.php?uid=<?=$_GET[uid]?>">Place Order for Auction</a></li>
		<? }else{?> 
			<li><a href="upload.php?uid=<?=$_GET[uid]?>">Place Order for Auction</a></li>
		<? } ?>
		<li><a href="transfer_history.php?uid=<?=$_GET[uid]?>">Transfers Log</a></li>
		<? if ($user[user_admin] == "YES"){?>
			<li><a href="users.php?uid=<?=$_GET[uid]?>">User Management</a></li>
			<li><a href="settings.php?uid=<?=$_GET[uid]?>">Client Options</a></li>
		<? } ?>
		<li><a href="deadlines.php?uid=<?=$_GET[uid]?>">New Auction Deadlines</a></li> 
		<? if ($user[attorneys_id] == '21'){?>
			<li><a href="report-draper-md.php?uid=<?=$_GET[uid]?>">MD Print Cost</a></li>
			<li><a href="report-draper-dc.php?uid=<?=$_GET[uid]?>">DC Print Cost</a></li>       
		<? }?>
		<li><a href="mailto:patrick@hwestauctions.com">Send Feedback</a></li>
		<li><a href="log.php?uid=<?=$_GET[uid]?>">Activity Log</a></li>
</fieldset>
<?
include 'footer.php';
?>

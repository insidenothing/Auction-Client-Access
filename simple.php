...
<li><a href='http://mdwestserve.com/ps/ps_write_invoice.php?id=".$_GET[id]."'>Invoice</a></li>";
...
<? }?>
<pre>
File Received: <?=$data[date_received]?><br>
...
<li><a href="simple.php?id=<?=$_GET[id]?>&uid=<?=$_GET[uid]?>&reinvoice=1">Invoice (will open in popup window)</a></li>
<li><a href="http://mdwestserve.com/affidavits/download.php?id=<?=$_GET[id]?>" target="_Blank">Download Complete PDF Package.</a></li>
<? 
...
if ($d2 > 0){
$i=0;
echo "<form action='/affidavits/download.php' method='POST' target='_Blank'><input type='hidden' name='packet' value='$_GET[id]'>";
	while ($d3=mysql_fetch_array($r2, MYSQL_ASSOC)){
		...
		$file = str_replace('http://mdwestserve.com/ps/affidavits//','',$d3[affidavit]);
		$file = str_replace('http://mdwestserve.com/ps/affidavits/','',$file);
		echo "<input name='download[$i]' type='checkbox' value='$file'><a target='_blank' href='$href'>".strtoupper($data['name'.$d3[defendantID]]).": $d3[method]</a><br>";
		$i++;
	}
echo "<input type='submit' value='Download Selected into Package'></form>";
}

}elseif ($data[filing_status] == "CANCELLED" || $data[filing_status] == "DO NOT FILE"){ ?>
Not Ready for Sale.<br>
Service Cancelled
</pre>
<li><a href="simple.php?id=<?=$_GET[id]?>&uid=<?=$_GET[uid]?>&reinvoice=1">Invoice (will open in popup window)</a></li>
<? }else{ ?>
...
<? if ($data[process_status] != 'CANCELLED'){ ?>
	<li><a href="simple.php?id=<?=$_GET[id]?>&uid=<?=$_GET[uid]?>&cancel=1">Cancel Service</a></li>
<? }?>
<?
include 'footer.php';
?>

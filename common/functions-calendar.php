...
function fillblock3($block, $difference, $month, $year, $user, $end){ // run1, run2
    ...
    //$end = date('t'); // the end of the month
    if ($day >= "1" && $day <= $end){ // limit low to the first, limit high to th<a href="wysiwyg-web.js">wysiwyg-web.js</a>e end of the month
		$content = "$day<br>";
...
function fillblock4($block, $difference, $month, $year, $user, $end){ // cal1, cal2, cal3
	...
	$q = "SELECT * FROM activity_log WHERE user_id='1' AND action_on like '$date1%'";
	$r = @mysql_query($q); 
	$count = mysql_num_rows($r);
	$content .= "<tr><td style='text-align:left'><a href='daily_report.php?id=1&date=$date1'>Patrick:</a></td><td>$count</td></tr>";
